...
func TestParseSQL(t *testing.T) {
	tests := []struct {
		name            string
		sql             sqlparams.SQLQuery
		formatParamFunc sqlparams.FormatParamFunc
		expected        sqlparams.ParsedSQL
		expectError     bool
		expectedError   error
	}{
		// Basic cases
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT id FROM logs WHERE file_path LIKE $1 || '%';", sqlparams.NewParameters("file_path")),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users WHERE active = true", nil),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users WHERE id = $1", sqlparams.NewParameters("id")),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM orders WHERE account_id=$1 AND created_at>=$2", sqlparams.NewParameters("accountId", "since")),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM orders WHERE created_at >= $1 AND updated_at >= $1", sqlparams.NewParameters("since")),
		},
		// Dotted path placeholders (ADR-008)
		{
			...
			expected: sqlparams.NewParsedSQL("INSERT INTO events (user_id, payload) VALUES ($1, $2)", sqlparams.NewParameters("user.id", "body.event")),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM products WHERE id = $1 AND sku = $2", sqlparams.NewParameters("items[0].id", "items[1].sku")),
		},
		// Different database backends
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users WHERE id = ?", sqlparams.NewParameters("id")),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users WHERE id = @p1", sqlparams.NewParameters("id")),
		},
		// String literal skipping
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users WHERE name = 'John :id Doe' AND id = $1", sqlparams.NewParameters("id")),
		},
		{
		...
		// Comment skipping
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users -- WHERE id = :id\nWHERE active = true AND id = $1", sqlparams.NewParameters("id")),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users /* WHERE id = :id */ WHERE active = true AND id = $1", sqlparams.NewParameters("id")),
		},
		// Error cases
		{
			...
			formatParamFunc: nil,
			expectError:     true,
			expectedError:   sqlparams.ErrFormatParamFuncRequired,
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users WHERE id = :123invalid", nil),
		},
		{
			...
			expectError:   true,
			expectedError: sqlparams.ErrInvalidPlaceholderName,
		},
...
ORDER BY u.created_at DESC`,
			...
			expected: sqlparams.NewParsedSQL(
				`SELECT u.*, p.title
FROM users u
LEFT JOIN posts p ON u.id = p.user_id
WHERE u.created_at >= $1
  ...
  AND u.org_id = $4
ORDER BY u.created_at DESC`,
				sqlparams.NewParameters("filters.since", "search.name", "search.email", "auth.org_id")),
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := sqlparams.ParseSQL(tt.sql, tt.formatParamFunc)
...
func TestParseSQL_EdgeCases(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM users WHERE name = 'O''Brien :id' AND id = $1", sqlparams.NewParameters("id")),
		},
		{
			...
			expected: sqlparams.NewParsedSQL("SELECT * FROM `users` WHERE `user-id` = $1", sqlparams.NewParameters("id")),
		},
		{
		...
		},
		{
			...
			expected: sqlparams.NewParsedSQL(
				...
				sqlparams.NewParameters("userId"),
			),
		},
		{
...
func TestParseSQL_NoInfiniteLoops(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			go func() {
				_, _ = sqlparams.ParseSQL(tt.sql, func(i int) string {
					return "?"
				})
				close(done)
			}()
